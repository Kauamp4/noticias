...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // Seleciona somente as notícias pesquisáveis.
    const noticias = Array.from(
        document.querySelectorAll(
            ".noticia-principal, .destaque-card, .card-noticia, .noticia-horizontal, .inovacao-destaque"
        )
    );
    ...
    function executarBusca() {
        ...
        noticias.forEach((noticia) => {
            const textoVisivel = noticia.textContent || "";
            const textoBusca = noticia.dataset.search || "";
            const categorias = noticia.dataset.category || "";

            const conteudo = normalizarTexto(
                `${textoVisivel} ${textoBusca} ${categorias}`
            );

            const corresponde = termo === "" || conteudo.includes(termo);

            noticia.hidden = !corresponde;
        ...
        // Oculta seções que ficaram sem notícias visíveis.
        document.querySelectorAll("section.secao").forEach((secao) => {
            const noticiasDaSecao = secao.querySelectorAll(
                ".card-noticia, .noticia-horizontal, .inovacao-destaque"
            );

            if (noticiasDaSecao.length === 0) {
                return;
            }

            const possuiNoticiaVisivel = Array.from(noticiasDaSecao).some(
                (noticia) => !noticia.hidden
            );

            secao.hidden = termo !== "" && !possuiNoticiaVisivel;
        });

        // Trata a manchete separadamente.
        const manchete = document.querySelector(".manchete");

        if (manchete) {
            const noticiasDaManchete = manchete.querySelectorAll(
                ".noticia-principal, .destaque-card"
            );

            const possuiNoticiaVisivel = Array.from(noticiasDaManchete).some(
                (noticia) => !noticia.hidden
            );

            manchete.hidden = termo !== "" && !possuiNoticiaVisivel;
        }

        limparBusca.classList.toggle("visivel", termoOriginal !== "");
        ...
        resultadoBusca.textContent =
            encontrados === 0
                ? `Nenhuma notícia encontrada para "${termoOriginal}".`
